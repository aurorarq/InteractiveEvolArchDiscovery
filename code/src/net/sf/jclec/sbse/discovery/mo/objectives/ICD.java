...
public class ICD extends Metric {
	...
	protected void prepare(Individual solution) {
		...
		// Compute c_out
		for(int i=0; i<numberOfComponents; i++){
			c_out[i] = solution.getNumberOfProvided(i) + solution.getNumberOfRequired(i);
		}
		...
		for(int i=0; i<numberOfComponents; i++){
			if(c_out[i]!=0){
				...
				classesRatio = ((double)nClasses - (double)solution.getNumberOfClasses(i))/(double)nClasses;
	...
	protected IFitness compute(Individual solution) {
		int nComponents = solution.getNumberOfComponents();
		double avg = 0.0;
